  ...
  try {
    ...
    for (const eng of engagements) {
      const counts = await db
        ...
        .leftJoin(
          riskScoresTable,
          eq(riskScoresTable.entryId, journalEntriesTable.id),
        )
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const scores =
        entryIds.length > 0
          ? await db
              ...
              .where(
                sql`${riskScoresTable.entryId} = ANY(ARRAY[${sql.raw(entryIds.join(",") || "NULL")}]::int[])`,
              )
          : [];
      ...
      const avgScore =
        scores.length > 0
          ? scores.reduce((sum, s) => sum + parseFloat(s.totalScore), 0) /
            scores.length
          : 0;
      ...
      const userStats: Record<
        string,
        { count: number; high: number; totalAmount: number; totalScore: number }
      > = {};
      for (const entry of entries) {
        const score = scoreMap.get(entry.id);
        if (!userStats[entry.postedBy]) {
          userStats[entry.postedBy] = {
            count: 0,
            high: 0,
            totalAmount: 0,
            totalScore: 0,
      ...
      const topRiskyUsers = Object.entries(userStats)
        ...
        .sort(
          (a, b) =>
            b.highRiskCount - a.highRiskCount || b.avgScore - a.avgScore,
        )
        .slice(0, 10);
        ...
        entryIds.length > 0
          ? ((
              await db
                ...
                .from(aiExplanationsTable)
                .where(
                  sql`${aiExplanationsTable.entryId} = ANY(ARRAY[${sql.raw(entryIds.join(",") || "NULL")}]::int[])`,
                )
            )[0]?.count ?? 0)
          : 0;
      ...
      const recentHighRiskScores = scores
        ...
        .slice(0, 5);
      const recentHighRisk = recentHighRiskScores
        .map((s) => {
          ...
          return entry
            ? {
                ...
                riskScore: { ...s, totalScore: parseFloat(s.totalScore) },
              }
            : null;
        })
        .filter(Boolean);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const userStats: Record<
        string,
        { total: number; high: number; totalScore: number; totalAmount: number }
      > = {};
      for (const e of entries) {
        const s = scoreMap.get(e.id);
        if (!userStats[e.postedBy])
          userStats[e.postedBy] = {
            total: 0,
            high: 0,
            totalScore: 0,
            totalAmount: 0,
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const avgDeviation =
        digits.reduce((sum, d) => sum + d.deviation, 0) / digits.length;
        ...
        .map(([key, grp]) => {
          ...
          return {
            ...
            entries: grp.map((e) => ({
              ...
              riskScore: scoreMap.get(e.id) ?? null,
            })),
